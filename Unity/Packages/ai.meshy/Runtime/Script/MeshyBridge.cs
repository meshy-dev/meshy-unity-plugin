...
public class MeshyBridge : MonoBehaviour
{
    ...
    private Queue<MeshTransfer> importQueue = new();
    ...
    public void StartServer()
    {
        ...
        _serverStop = false;
        serverThread = new(RunServer) { IsBackground = true };
        serverThread.Start();
    }
    ...
    void RunServer()
    {
        listener = new(IPAddress.Any, 5326);
        listener.Server.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.ReuseAddress, true);
        listener.Start();

        guardThread = new(GuardJob) { IsBackground = true };
        guardThread.Start();
